...
class UterusSegmentationTrainer:
    """Wrapper around Detectron2's DefaultTrainer configured for uterus segmentation."""
    ...
    def setup_config(self, params: Optional[dict] = None) -> None:
        ...
        # Évaluation toutes les 400 itérations
    ...
    def register_datasets(
        ...
        """Enregistre les datasets COCO pour Detectron2"""
    ...
    def train(self) -> DefaultTrainer:
        ...
        # Early stopping basé sur total_loss toutes les 400 itérations
        early_stopping_cfg = {
            "patience": 40,
            "maximize": False,
            "min_delta": 1e-4,
            "warmup_iters": self.cfg.TEST.EVAL_PERIOD,  # 400 itérations
        }

        trainer = EarlyStoppingTrainerWithCheckpoints(
            self.cfg,
            early_stopping=early_stopping_cfg,
            save_every_n_iters=400,  # checkpoints toutes les 400 itérations
        )

        trainer.resume_or_load(resume=False)

        # Hook pour afficher les métriques de validation toutes les 400 itérations
            ...
            def after_step(self):
                if (self.trainer.iter + 1) % self.trainer.cfg.TEST.EVAL_PERIOD == 0:
                    ...
                    evaluator = COCOEvaluator(
                        "uterus_val", self.trainer.cfg, False, output_dir=self.trainer.cfg.OUTPUT_DIR
                    )
